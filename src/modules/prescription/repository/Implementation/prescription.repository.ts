  ...
  implements IPrescriptionRepository
{
  ...
  async createPrescription(
    ...
    const detailedPrescription = (await this.PrescriptionModel.findById(
      result._id,
    )
      .populate('patientId', 'name gender date_of_birth')
      .populate('doctorId', 'name specialistation')
      .exec()) as unknown as CreatePrescriptionPdfDto;

    if (
      prescription.prescriptionForId.toString() !==
      prescription.patientId.toString()
    ) {
      let relativeData = await this.userRepo.getRelativeData(
        result.patientId.toString(),
        result.prescriptionForId,
      );
      ...
      detailedPrescription.patientId.date_of_birth =
        relativeData.patients[0].dateOfBirth;
    }
    ...
    const { key, isPublic } =
      await this.prescriptionService.generatePrescriptionPDF(
        detailedPrescription,
      );
    await this.PrescriptionModel.updateOne(
      { _id: result._id },
      { $set: { prescriptionPdfUrl: key } },
    );
    return result;
  }
  ...
  async getPrescriptionsByPatientId(
    ...
    const prescriptions = await this.PrescriptionModel.find({ patientId })
      ...
      .limit(limit)
      .exec();
    const totalDocs = await this.PrescriptionModel.countDocuments({
      patientId,
    });
    return {
      prescriptions,
  ...
  async updatePrescription(data: UpdatePrescriptionDto): Promise<{
    acknowledged: boolean;
    matchedCount: number;
    modifiedCount: number;
  }> {
    return await this.PrescriptionModel.updateOne(
      { _id: data._id },
      { $set: data },
    );
  }
