...
export class AuthService {
  ...
  generateDoctorTokens(payload: {
    ...
    const doctorAccessToken = this.jwtService.sign(payload, {
  ...
  async resendOtp(email: string, role: UserRole) {
    const checkOtp = await this.OtpRepository.findOne({ email });
  ...
  async logout(_id: string, res) {
    try {
      ...
      throw new RequestTimeoutException(
        'Database not responding. Please try again later.',
      );
    }
  }
  ...
  async doctorLogin(email: string, password: string, res) {
    ...
    const { doctorAccessToken, doctorRefreshToken } =
      await this.generateDoctorTokens(payload);
  ...
  async verifyDoctor(id: string, data: Partial<UpdateDoctorDto>) {
    ...
    await this.doctorService.updateDoctorDocuments(id, {
  ...
  async adminLogin(email: string, password: string, res) {
    ...
    const { adminAccessToken, adminRefreshToken } =
      await this.generateAdminTokens(payload);
  ...
  async adminRenewTokens(admin, res) {
    ...
    if (adminData) {
      ...
      const { adminAccessToken, adminRefreshToken } =
        this.generateAdminTokens(payload);
      console.log(
        'Reached renew admin refresh token and created refresha nd accesstoken',
      );
      res.cookie('adminRefreshToken', adminRefreshToken, {
        httpOnly: true,
        secure: true,
