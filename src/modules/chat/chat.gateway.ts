...
export class ChatGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleConnection(client: Socket) {
    ...
    console.log(
      `User ${userId} (Type: ) connected with socket for chatt ${client.id}`,
      this.connectedUsers.keys(),
    );
  ...
  handleVideoCallReq(
    client: Socket,
    {
      ...
      to,
      videoCallId,
    }: { from: string; to: string; videoCallId: string },
  ) {
    const receiverSocketId = this.connectedUsers.get(to);
    if (receiverSocketId) {
      console.log(
        'going to emit video call req to the online user',
        receiverSocketId,
      );
      this.server
        .to(receiverSocketId)
        .emit('VideoCallInitiated', { from, to, videoCallId });
    } else {
      this.server
        .to(client.id)
        .emit('RecieverNotOnline', { from, to, videoCallId });
    }
  }
  ...
  handleVideoCallAccepted(
    ...
    if (receiverSocketId) {
      this.server
        .to(receiverSocketId)
        .emit('VideoCallAccepted', { from, to, videoCallId });
    } else {
      this.server
        .to(client.id)
        .emit('RecieverNotOnline', { from, to, videoCallId });
  ...
  handleVideoCallRejected(
    ...
    if (receiverSocketId) {
      ...
      this.server
        .to(receiverSocketId)
        .emit('VideoCallRejected', { from, to, videoCallId });
    } else {
      this.server
        .to(client.id)
        .emit('RecieverNotOnline', { from, to, videoCallId });
