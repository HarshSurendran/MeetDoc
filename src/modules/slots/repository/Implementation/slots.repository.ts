...
import { ISlptsRepository } from '../Interface/ISlots.repository';

@Injectable()
export class SlotsRepository implements ISlptsRepository {
  ...
  async addSlot(slotData: Partial<CreateSlotDto>): Promise<SlotDocument> {
    try {
      ...
      throw new InternalServerErrorException(
        "Can't create slots now. Please try again later.",
      );
    }
  }

  async findSlotAndUpdateWithSession(
    slotId: string,
    slotData: Partial<UpdateSlotDto>,
    session,
  ...
  async updateSlot(
    ...
    const updateStatus = await this.SlotModel.updateOne(
      { _id: slotId },
      { $set: slotData },
    ).exec();
    if (!updateStatus.acknowledged) {
      ...
      throw new NotFoundException(
        `Didnt match any slots with this Id ${slotId}`,
      );
    }
    return updateStatus;
  }
  ...
  async getSlotsByDoctorId(doctorId: string): Promise<SlotDocument[] | null> {
    try {
      ...
      if (!(error instanceof NotFoundException)) {
        console.log(
          `Unexpected error during fetching slots of doctor ${doctorId}`,
        );

        throw new InternalServerErrorException(
          "Can't fetch slots of this doctor now. Please try later.",
        );
      }
      throw error;
    }
  ...
  async deleteSlotsOlderThan3Months(currentTime: Date): Promise<{
    acknowledged: boolean;
    deletedCount: number;
  }> {
    try {
      return await this.SlotModel.deleteMany({
        StartTime: { $lt: currentTime },
      });
    } catch (error) {
      console.log(
        'Unexpected error occured while fetching slots older than 3 months',
      );
    }
  }

  async getMonthlySlotsByDoctorId(doctorId: string): Promise<
    {
      _id: {
        year: number;
        month: number;
  ...
  > {
    try {
      ...
      console.log(
        'Unexpected error occured while fetching monthly slots by doctor id',
      );
    }
  }
}
